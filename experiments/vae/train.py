...
model_dir = (
    RESOURCES / "models" / "vae" / (datetime.now().strftime("%Y%m%d-%H%M%S") + "l2")
)
# model_dir = RESOURCES / "models" / "vae" / "test"
tb_logger = TBLogger(model_dir / "logs")

config = VAEConfig(hidden_dims=(256, 128), latent_dim=2, lr=1e-3, batch_size=32)
...
try:
    model.train_loop(
        ...
        on_nbatch_end=callback_save_zdist,
    )
finally:
